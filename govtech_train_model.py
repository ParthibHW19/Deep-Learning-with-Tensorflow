...
def train_cnn_rnn():
	...
	x_, y_, vocabulary, vocabulary_inv, df, labels = govtech_data_helper.load_data(input_file)

	training_config = sys.argv[2]
	params = json.loads(open(training_config).read())

	# Assign a 300 dimension vector to each word
	word_embeddings = govtech_data_helper.load_embeddings(vocabulary)
	...
	with graph.as_default():
		...
		with sess.as_default():
			...
			train_batches = govtech_data_helper.batch_iter(list(zip(x_train, y_train)), params['batch_size'], params['num_epochs'])
			...
			for train_batch in train_batches:
				...
				if current_step % params['evaluate_every'] == 0:
					dev_batches = govtech_data_helper.batch_iter(list(zip(x_dev, y_dev)), params['batch_size'], 1)
			...
			test_batches = govtech_data_helper.batch_iter(list(zip(x_test, y_test)), params['batch_size'], 1, shuffle=False)
